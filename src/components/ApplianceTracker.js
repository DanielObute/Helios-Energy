...
const defaultAppliances = [
  { id: 1, name: 'Air Conditioner', icon: '❄️', watts: 2000, hoursPerDay: 8, category: 'Cooling' },
  { id: 2, name: 'Refrigerator', icon: '🧊', watts: 150, hoursPerDay: 24, category: 'Kitchen' },
  { id: 3, name: 'Washing Machine', icon: '🫧', watts: 500, hoursPerDay: 1, category: 'Laundry' },
  { id: 4, name: 'Water Heater', icon: '🚿', watts: 3000, hoursPerDay: 2, category: 'Heating' },
  { id: 5, name: 'TV (LED 50")', icon: '📺', watts: 100, hoursPerDay: 5, category: 'Entertainment' },
  { id: 6, name: 'Lighting (10 bulbs)', icon: '💡', watts: 100, hoursPerDay: 6, category: 'Lighting' },
  { id: 7, name: 'Microwave', icon: '📡', watts: 1200, hoursPerDay: 0.5, category: 'Kitchen' },
  { id: 8, name: 'Laptop', icon: '💻', watts: 65, hoursPerDay: 8, category: 'Electronics' },
  { id: 9, name: 'Electric Oven', icon: '🍳', watts: 2400, hoursPerDay: 1, category: 'Kitchen' },
  { id: 10, name: 'Dishwasher', icon: '🍽️', watts: 1800, hoursPerDay: 1, category: 'Kitchen' },
]
...
function ApplianceTracker() {
  ...
  const maxKwh = parseFloat(getDailyKwh(sorted[0]?.watts, sorted[0]?.hoursPerDay))
  ...
  const addAppliance = (e) => {
    ...
    setAppliances([...appliances, newOne])
  ...
  const updateHours = (id, hours) => {
    setAppliances(appliances.map(a => a.id === id ? { ...a, hoursPerDay: parseFloat(hours) || 0 } : a))
  }
      ...
      <div className="container">
        ...
        <div className="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
          <div>
            ...
            <p style={{ color: '#6b7280', fontSize: '14px', margin: '0' }}>Track which appliances consume the most electricity</p>
          </div>
                ...
                {appliances.map(a => (
                  <tr key={a.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <td style={{ padding: '12px', border: 'none' }}>
                      <input
                        type="number"
                        value={editHours[a.id] !== undefined ? editHours[a.id] : a.hoursPerDay}
                        step="0.5"
                        min="0"
                        max="24"
                        onChange={(e) => { setEditHours({ ...editHours, [a.id]: e.target.value }); updateHours(a.id, e.target.value) }}
